<#
 .SYNOPSIS
  Exports list of VM Templates to a CSV file.

 .DESCRIPTION
  Exports list of Virtual Machine Templates to a CSV file.
  Related configuration files: settings.ps1
  Version: 1.1

 .PARAMETER OutPath
  Path and name of output file. If not specified, <script_path>\vmtplout.txt
  is used. If EsxHost argument is specified too, filename defaults to 
  vmtplout-<EsxHost>.txt.

 .PARAMETER EsxHost
  Name of ESX Host where Templates reside. If not specified, all the VMs on all
...
if ($OutPath) {
    ...
    if ($EsxHost) {
        $PathOut = $scriptdir + "\vmtplout-" + $EsxHost + ".txt"
    } else {
        $PathOut = $scriptdir + "\vmtplout.txt"
    }
}
...
Function List-VM-byClst {
    PROCESS {
		Write-Output "Gathering VM Information"
		$Report = @()

		foreach ($vhost in (Get-VMHost)) {
			foreach ($vm in (Get-Template -Location $vhost)) {
				$VMHostName = $vhost.Name
				...
				$TotalHDDs = ($GlobalHDDinfo |Measure-Object -Sum -Property CapacityKB).Sum
				...
				#IP Address
				#$IPs = $vm.Guest.IPAddress -join "," #$vm.Guest.IPAddress[0] <#it will take first ip#>
				#$iplist = @()
				...
				$Vmresult | add-member -MemberType NoteProperty -Name "EsxiHost" -Value $vhost.Name
				$Vmresult | add-member -MemberType NoteProperty -Name "VMName" -Value $vm.Name
				$Vmresult | add-member -MemberType NoteProperty -Name "PowerState" -Value "Template"
				$Vmresult | add-member -MemberType NoteProperty -Name "Total-HDD(GB)" -Value ($TotalHDDs/1048576)
				$Vmresult | add-member -MemberType NoteProperty -Name "Datastore" -Value $datastore
				$Vmresult | add-member -MemberType NoteProperty -Name "vCPU" -Value $vm.ExtensionData.config.hardware.NumCPU
				...
				$Vmresult | add-member -MemberType NoteProperty -Name "RAM(GB)" -Value ($vm.ExtensionData.config.hardware.MemoryMB/1024)
