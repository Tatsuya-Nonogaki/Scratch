        ...
        if ($TrimOUList -and $trimCount -gt 0) {
            ...
            if ($ouNames.Count -ge $trimCount) {
                ...
                for ($i = 0; $i -lt $trimCount; $i++) {
                    if ($($ouNamesTail[$i]).ToString().ToLower() -ne $($TrimOUList[$i]).ToString().ToLower()) {
                        $match = $false
                        break
                    }
                }
                if ($match) {
                    # Remove the matching OUs from the end, but if nothing remains, set to empty array
